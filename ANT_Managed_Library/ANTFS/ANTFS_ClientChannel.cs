...
#pragma warning restore 1591
        ...
        #region Variables
        const int ANTFS_CHANNEL_DEFAULT_PERIOD = 8192;    // Default channel period for ANT-FS, in 1/32768 seconds (4Hz)
        bool bInitialized;
            ...
            if (bInitialized)
            {
                ...
                if (ResponseThread != null && ResponseThread.IsAlive)
                {
                    ...
                    try
                    {
                        ...
                        {
                            try { ResponseThread.Interrupt(); } catch { }
                            if (!ResponseThread.Join(2000))
                            {
                                System.Diagnostics.Debug.Fail("Response thread did not terminate after interrupt.");
                            }
                        }
                    }
                    catch { }
